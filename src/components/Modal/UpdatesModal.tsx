    ...
    >
      <div className="modal-panel w-[min(560px,100%)] max-h-[80vh] rounded-2xl flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[rgba(38,23,50,.12)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#7D7482]">Текущая версия: {currentVersion}</p>
            </div>
          </div>
          <button
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {isCurrentVersion && (
                          <span className="text-xs font-bold bg-[#AEA989] text-[#261732] px-2 py-0.5 rounded-full">
                            ТЕКУЩАЯ
                          </span>
                        )}
                        {release.prerelease && (
                          <span className="text-xs font-bold bg-[#C58C75] text-[#261732] px-2 py-0.5 rounded-full">
                            ПРЕРЕЛИЗ
                          </span>
                        )}
                      </div>
                      ...
                      {release.body && (
                        <p className="text-xs text-[#261732] mt-2 whitespace-pre-line leading-relaxed">
                          {release.body}
                        </p>
                      )}
                    </div>
                  ...
                  {exeAsset && (
                    <div className="mt-3 flex items-center gap-3 pt-3 border-t border-[rgba(38,23,50,.08)]">
                      ...
                      <button
                        className="ml-auto h-8 px-4 bg-[#261732] text-[#E7E0D6] rounded-lg text-xs font-semibold flex items-center gap-1.5 hover:bg-[#1a1024]"
                  ...
                  {!exeAsset && release.assets.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-[rgba(38,23,50,.08)]">
                      <p className="text-xs text-[#7D7482] mb-2">Доступные файлы:</p>
                      <div className="flex flex-wrap gap-2">
                        {release.assets.map(asset => (
                          <button
                            key={asset.name}
                            className="h-7 px-3 border border-[rgba(38,23,50,.12)] rounded-lg text-xs font-medium flex items-center gap-1 hover:bg-[#F3EFE9]"
        ...
        <div className="px-6 py-4 border-t border-[rgba(38,23,50,.12)]">
          <button
            className="w-full h-10 border border-[rgba(38,23,50,.12)] rounded-lg text-xs font-semibold flex items-center justify-center gap-2 hover:bg-[#F3EFE9]"
